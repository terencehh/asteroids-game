...
function asteroids() {
  ...
  const mainObservable = mainInterval.takeUntil(mainInterval.filter(
  ...
  function animateAsteroids(asteroid: Elem) {
    ...
    const animateAsteroidsObservable = Observable.interval(100).map(({}) => ({shipValues: 
      /translate\((\d+) (\d+)\) rotate\((\d+)\)/.exec(g.attr("transform")), gameStats, asteroid}))
      .filter(({gameStats}) => gameStats.startingLives > 1)
